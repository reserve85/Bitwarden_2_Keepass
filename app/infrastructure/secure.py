"""RAM hygiene helpers for secrets.

Guarantees are scoped honestly: Python ``str``/``bytes`` are immutable and
cannot be physically wiped - the enforceable promise is "no extra copies, no
persistence, no logging". These helpers minimise copies, keep secrets in one
mutable ``bytearray`` and best-effort wipe it in caller ``finally`` paths. All
functions are guaranteed not to raise on their documented inputs.
"""
